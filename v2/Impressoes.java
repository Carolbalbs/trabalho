...
public class Impressoes {
		...
		if (livro.getExemplares().isEmpty()) {
			...
			for (Exemplar exemplarAtual : livro.getExemplares()) {
				...
				if (exemplarAtual.isDisponivel()) {
					...
					String data = new SimpleDateFormat("dd/MM/yyyy").format(exemplarAtual.getEmprestimoAtual().getData());
					System.out.println("Data do empréstimo: " + data);
					System.out.println("Tempo previsto para devolução: " + exemplarAtual.getEmprestimoAtual().getUsuario().getTempoDeEmprestimo() + " dias\n");
				}
			}
		}
	}
	...
	public static void consultaUsuario(Usuario usuario) {
		...
		if (usuario.getEmprestimosAtuais().size() > 0) {
			...
			for (Emprestimo emprestimoAtual : usuario.getEmprestimosAtuais()) {
				...
				String data = new SimpleDateFormat("dd/MM/yyyy").format(emprestimoAtual.getData());
				System.out.println("Data do empréstimo: " + data);
		...
		if (usuario.getReservaAtuais().size() > 0) {
			...
			for (Reserva reservaAtual : usuario.getReservaAtuais()) {
				...
				String data = new SimpleDateFormat("dd/MM/yyyy").format(reservaAtual.getData());
		...
		if (usuario.getReservaPassados().size() > 0) {
			System.out.println("Reservas finalizadas:\n");
			for (Reserva reservaAtual : usuario.getReservaPassados()) {
				System.out.println("Título do livro: " + reservaAtual.getLivro().getTitulo());
				String data = new SimpleDateFormat("dd/MM/yyyy").format(reservaAtual.getData());
				System.out.println("Data da reserva: " + data);
				System.out.println("Status: Finalizado\n");
	...
	public static void usuarioNaoProfessor() {
		System.out.println("Apenas professores tem acesso a essa opção!\n");
	}
